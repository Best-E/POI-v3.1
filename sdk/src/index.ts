...
function toIdentityType(raw: number): IdentityType {
  if (raw === 1) return 'Human';
  if (raw === 2) return 'Agent';
  return 'None';
}
  ...
  return {
    verify: async ({address, claimedId}: {address: Address, claimedId?: string}): Promise<CheckResult> => {
      if (!claimedId) {
        ...
        }) as unknown as [string, number, number, boolean, boolean];
      ...
      if (claimedId.startsWith('@') && idTypeRaw !== 1) return { status: 'type_mismatch', actualId, idType: 'Agent', riskLevel: 'danger', canProceed: false, message: '⛔ TYPE ERROR: Address is Agent but you entered @human' };
      if (claimedId.startsWith('#') && idTypeRaw !== 2) return { status: 'type_mismatch', actualId, idType: 'Human', riskLevel: 'danger', canProceed: false, message: '⛔ TYPE ERROR: Address is Human but you entered #agent' };
